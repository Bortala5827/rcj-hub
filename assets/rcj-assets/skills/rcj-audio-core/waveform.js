...
export function renderWave(canvas, peaks, opts = {}) {
  const {
    ...
    headWidth = 2,
    // v6 — 幅度增强（回放波形也让轻声/弱段更可见，默认关，手动开）
    normalize = false,       // 自动归一：把整段峰值拉到满幅，避免整体贴地
    gain = 1,                // 线性放大
    gamma = 1,               // <1 抬升中小值（轻声更可见）
    fill = 0.92,             // 柱体占据半屏的比例（与原硬编码值一致，保持默认外观）
  } = opts;
  ...
  // 自动归一化系数（以整段最大值为满幅参考，避免整体贴地）
  let scale = 1;
  if (normalize) {
    let mx = 0;
    for (let mi = 0; mi < n; mi++) if (peaks[mi] > mx) mx = peaks[mi];
    if (mx > 0.0015) scale = 0.96 / mx;
  }
  for (let i = 0; i < n; i++) {
    let v = peaks[i] * scale * gain;
    if (gamma !== 1) v = Math.pow(v, gamma);
    v = Math.max(minBar, Math.min(1, v));
    const bh = v * (h / 2) * fill;
    const x = i * bw;
...
export function mountLiveBars(canvas, analyser, opts = {}) {
  const {
    ...
    // v7 — AGC 自适应增益（解决「柱子太矮、跳动不明显」，默认关，手动开）
    agc = false,             // 跟踪近期峰值当满幅参考 → 说话时最高柱自动顶到满高
    agcFloor = 0.16,         // 最低满幅参考（越小越灵敏，太小会放大底噪）
    agcAttack = 0.4,         // 峰值上升跟随速度
    agcRelease = 0.992,      // 峰值回落速度（越接近 1 越平稳）
    gain = 1,                // 线性放大
    gamma = 1,               // <1 抬升中小值，让轻声也看得见
    noiseGate = 0.02,        // 静音门限，低于此值当 0，防底噪满屏
  } = opts;
  ...
  let agcPeak = agcFloor;         // AGC 满幅参考（跟踪近期峰值）
  ...
  const draw = () => {
    ...
    // ── AGC：以近期峰值为满幅参考，让说话时最高柱顶到画布顶部 ──
    if (agc) {
      let frameMax = 0;
      for (let fk = 0; fk < N; fk++) if (vals[fk] > frameMax) frameMax = vals[fk];
      if (frameMax > agcPeak) agcPeak += (frameMax - agcPeak) * agcAttack;
      else agcPeak = Math.max(agcFloor, agcPeak * agcRelease);
    }
    ...
    for (let k = 0; k < N; k++) {
      let raw = vals[k];
      if (raw < noiseGate) raw = 0;
      let v = agc ? raw / (agcPeak || 1) : raw;
      v = v * gain;
      if (gamma !== 1) v = Math.pow(v, gamma);
      if (pulse) v = v * beat;
      if (wobble) {
