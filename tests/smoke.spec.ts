...
function attachListeners(page: Page) {
  ...
  page.on('console', (msg) => {
    ...
    // 明确标记：CF Pages Functions 运行时依赖（埋点 beacon /api/track）。
    // 本地静态服务与跨源测试下该请求会失败（线上经 curl 实测返回 200），
    // 属环境依赖而非站点 Bug，故排除，但不静默——已在 TESTING.md 记录。
    if (KNOWN_RUNTIME_PATHS.some((p) => url.includes(p))) return;
  ...
  page.on('requestfailed', (req) => {
    const u = req.url();
    if (KNOWN_RUNTIME_PATHS.some((p) => u.includes(p))) return; // 标记但不算失败
...
/** 取完整 URL（兼容 path / url 两种声明） */
function fullUrl(site: SitePage): string {
  return site.url ?? site.path!;
}

test.describe('RCJ Hub 主站 + 子路径 · 页面 / 控制台 / 网络', () => {
...
// INCLUDE_ECOSYSTEM=false（CI/默认）→ 用 describe.skip 整体跳过；=true → 正常运行
